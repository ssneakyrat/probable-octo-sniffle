  ...
  return (
    <div className="flex flex-col items-center p-4 bg-gray-100 rounded-lg shadow-md w-full max-w-4xl mx-auto select-none">
      ...
      <div className="flex space-x-2 mb-4">
        <button 
          className={`px-4 py-2 rounded ${editorMode === 'select' ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}
          onClick={() => setEditorMode('select')}
        >
          Select Mode
        </button>
        <button 
          className={`px-4 py-2 rounded ${editorMode === 'draw' ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}
          onClick={() => setEditorMode('draw')}
        >
          Draw Mode
        </button>
        <button 
          className={`px-4 py-2 rounded ${editorMode === 'delete' ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}
